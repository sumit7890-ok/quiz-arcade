...
const quotes = [
    ...
    "Play. Learn. Conquer. ⚡",
    "Challenge Yourself & Unlock Knowledge. 🚀",
    "Ready to prove your skills? Let's play! 🎮",
    "Every Question Counts. Make It Matter. ⭐",
    "Knowledge is Power. Show Yours. 💡",
    "Test Your Brain. Beat the Clock. ⏱️",
    "Learn Something New Today. 📚",
    "Only the Curious Become Champions. 🏆",
    "One Quiz. Endless Possibilities. ✨"
];

document.getElementById("randomQuote").textContent =
    quotes[Math.floor(Math.random() * quotes.length)];

function createFloatingQuestionMarks() {
  const layer = document.querySelector(".floating-bg");
  if (!layer) return;
  ...
  for (let i = 0; i < markCount; i++) {
    ...
    const size = getRandomInt(28, 96);
    const x = getRandomInt(-4, 98);
    const y = getRandomInt(-6, 96);
    const driftX = getRandomInt(-70, 70);
    const driftY = getRandomInt(-90, 65);
    const duration = getRandomInt(9000, 22000);
    const delay = getRandomInt(-22000, 0);
    const rotate = getRandomInt(-28, 28);
    const alpha = (Math.random() * 0.16 + 0.36).toFixed(2);

    mark.textContent = "?";
    mark.style.setProperty("--size", `${size}px`);
    mark.style.setProperty("--x", `${x}%`);
    mark.style.setProperty("--y", `${y}%`);
    mark.style.setProperty("--drift-x", `${driftX}px`);
    mark.style.setProperty("--drift-y", `${driftY}px`);
    mark.style.setProperty("--drift-x-reverse", `${driftX * -1}px`);
    mark.style.setProperty("--drift-y-reverse", `${driftY * -1}px`);
    mark.style.setProperty("--duration", `${duration}ms`);
    mark.style.setProperty("--delay", `${delay}ms`);
    mark.style.setProperty("--rotate", `${rotate}deg`);
    mark.style.setProperty("--alpha", alpha);
